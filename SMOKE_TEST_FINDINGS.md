...
**Status:** ✅ SOLVED (2026-07-04, post-reboot) — **there was never any instability.** Win11 26200 records the user's choice under `UrlAssociations\<scheme>\UserChoiceLatest\ProgId` (value `ProgId`, plus a `Hash` on the key) and leaves legacy `UserChoice` stale forever (still `ChromeHTML` here). Both "reverts"/"failed sets" were reads of the dead key. Proof: `UserChoiceLatest` = `FinickyURL` for http+https; the owner's choice **survived a reboot** with no re-set; 13 post-boot Discord clicks all routed through Finicky. **Code fix landed:** `browser_windows.go` `getDefaultHandlerForURLScheme` now prefers `UserChoiceLatest\ProgId` and falls back to `UserChoice` (older builds); post-fix log reads `Finicky is the default browser`. **Tier 1 = fully complete.** Note: owner-perceived "everything opened in Edge" was correct F9 behavior (no rule matched; default = Edge).
